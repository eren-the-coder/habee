...
export function HabitDetail({ habit, onBack, onEdit, onDelete, onToggleDay }: HabitDetailProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 min-h-screen pb-8">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={onBack}
          className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-all"
        >
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onDelete}
            className="p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900 transition-all"
          >
      ...
      <div className="flex justify-center mb-4">
        <button
          ...
          className="flex items-center gap-2 px-5 py-2 rounded-full bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 transition-all"
        >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="font-bold text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 px-4 py-2 rounded-full capitalize">
          {periodLabel}
        </span>
        <button
          onClick={() => navigate(1)}
